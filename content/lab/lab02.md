...
这个 while 循环就是第 3 章讲的 **Agent Loop** 的全部本质。Pi 的 `pi-agent-core` 包里几千行代码，核心骨架和这一关一模一样。
...
**1. 工具有两半。** `tools` 数组是给模型看的"说明书"（叫什么名字、干什么用、参数长什么样）；`toolImpls` 是真正干活的函数。模型只输出"我想调 read_file、参数是这个 JSON"——**执行永远发生在你这边**。这就是第 5 章"工具系统"的核心事实。

**2. 循环的停止条件。** `while` 什么时候停？不是"任务完成"——模型没说这个词——而是**它这一轮没再请求工具**。这个判据写进了几乎所有生产级 Agent 的源码里，也是面试 30 题里第 3 章的原题。
...
- **路径穿越**：教学代码没校验 `filename`，模型（或被注入的文本）可以写 `../` 逃逸出工作目录——安全话题超出本专栏范围，Pi 的做法见第 5 章
...
- 第 3 章：Agent Loop——这一关的 while 循环就是它的最小实现
- 第 5 章：工具系统——工具定义、参数 Schema、结果回包的完整链路
