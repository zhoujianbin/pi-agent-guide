...
**1. system prompt 是行为契约。** 四条规则分别对应：先读后写（防盲改）、错误自愈、汇报节制、不猜文件名。你会惊讶于这么几行字对行为的约束力——第 3 章系统提示词工程讲的就是这个。
...
- 接入第 3 关的流式输出，消灭等待焦虑
- 挂上第 4 关的压缩器，支持长会话
- 加一个 `bash` 工具（先想想第 4 章讲的注入风险，至少加个命令白名单）
- 把 messages 落盘成 JSONL，下次启动恢复——这就是第 10 章会话文件格式的雏形
- 用 `pi-agent-core` 重写一遍，对照看看你手搓的版本少了什么（答案会告诉你第 2 章那些设计为什么存在）
...
把你 5 关写出的代码和 Pi 的四个包对一下：callModel ≈ `pi-ai`，Agent Loop ≈ `pi-agent-core`，REPL ≈ `pi-tui` + `pi-coding-agent`。然后打开 Pi 源码随便挑一个你实现过的函数读——**能预判作者的下一行，就算出师了**。
